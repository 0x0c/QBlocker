...
private func keyDownCallback(proxy _: CGEventTapProxy, type _: CGEventType, event: CGEvent, ptr _: UnsafeMutableRawPointer?) -> Unmanaged<CGEvent>? {
    ...
    if let bundleId = app.bundleIdentifier {
        ...
        case (.denyList, true), (.allowList, false):
...
private func keyUpCallback(proxy _: CGEventTapProxy, type _: CGEventType, event: CGEvent, ptr _: UnsafeMutableRawPointer?) -> Unmanaged<CGEvent>? {
    // If the command key wasn't used we can pass the event on
    let flags = event.flags
    guard flags.contains(.maskCommand) else {
        return Unmanaged<CGEvent>.passUnretained(event)
...
class KeyListener {
    ...
    static var delay: Int {
        UserDefaults.standard.integer(forKey: "delay")
    }

    var listMode: ListMode {
        ListMode(rawValue: UserDefaults.standard.integer(forKey: "listMode")) ?? .allowList
    }
    ...
    class func isCmdQActive(for app: NSRunningApplication) -> Bool {
        ...
        guard let items = children as? [Any], items.isEmpty == false else {
            return false
        }
        ...
        guard let menus = subMenus as? [Any], menus.isEmpty == false else {
            return false
        }
        ...
        guard let menuItems = entries as? [Any], menuItems.isEmpty == false else {
            return false
        }
